...
function encodeChars(characters=charactersJSON,sizes=[1]){
	...
	for(let sizeIndex=0; sizeIndex<sizes.length; sizeIndex+=1){
		...
		for(let charIndex=0; charIndex<chars.length; charIndex+=1){
			...
			process.stdout.write(`\rsize: ${size}, char: ${char}, ${charIndex+1}/${chars.length}    `);
			if(size>1){
				let newCharMap=[];
...
encodeChars(null,[1,2,3,4,5,6,7]);
